...
def ellipse(rxy, rz, beam_xy, beam_z, x, y, z, theta, s, straight):
    """Make a 3D ellipse.
...
def plot_beam_size(ax, twiss, survey, color, scale=1e3):
    ...
    pts = np.array([
        ellipse(sigx[i] * scale, sigy[i] * scale, x[i] * scale, y[i] * scale, sx[i], sz[i], sy[i], -theta[i], s[i], straight)
        for i in range(min_len)
    ])

    # Plot the envelopes
    surface = mesh_from_polygons(pts)
    ax.add_mesh(surface, color=color, opacity=1, show_edges=True)
...
ax.show_bounds(
    ...
    show_ylabels=True,
    show_zlabels=True,
    xtitle='X [cm]' if straight else 'X [m + cm]',
    ytitle='Z [m]' if straight else 'Z [m + cm]',
    ztitle='Y [cm]',
    location='outer',
)
title = ax.add_title(f'Toy Ring (3σ, 1.2GeV p+)')
